...
/**
 * 사람에 관한 값을 가린다 — 길이는 안 자른다. 응답 본문처럼 통째로 봐야 할 때.
 *
 * 한글(이름)뿐 아니라 **전화번호도 가린다.** 한글만 가리던 시절 긴 칸을
 * 통째로 찍었더니 학부모 번호가 그대로 나갔다. 숫자를 전부 가리면 학번·시각
 * 같은 조사에 필요한 값까지 못 보게 되므로, 전화번호 모양만 골라 가린다.
 */
function maskNames(s) {
  return String(s)
    .replace(/\b01\d[-\s]?\d{3,4}[-\s]?\d{4}\b/g, "010-****-****") // 휴대폰
    .replace(/\b0\d{1,2}[-\s]\d{3,4}[-\s]\d{4}\b/g, "0**-***-****") // 일반전화
    .replace(/[가-힣]/g, "●");
}
  ...
  if (args.includes("--calls")) {
    ...
    for (const q of queries) {
      ...
      try {
        ...
        // 칸 구성은 멀쩡한데 줄이 0개면, 구분자 인자가 비어서인 경우가 많다.
        // (등하원명단은 in_aopt로 전체/수업학생/무수업생/비원생을 가른다.)
        // 값을 몇 개 돌려보고 줄이 나오는 조합을 찾아준다 — 이름을 맞히느라
        // 다시 돌리게 하지 않으려는 것.
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(attendDate)) {
    ...
    if (cIdx.length === 0) {
      ...
      if (parsedAtt.rows.length > 0) {
        ...
        //
        // 예전엔 "사유처럼 보이는 칸이 없을 때만" 찔러봤는데, SMS_SEND에
        // 한글이 들어 있어 이미 찾은 줄 알고 건너뛰었다. 어설픈 짐작으로
        // 조사를 건너뛰느니 매번 두 번 더 물어보는 편이 낫다.
        ...
        for (const [path, fields] of [
          ...
          ["/job/attend_S03.aspx", { in_m_branch: branch, in_yyyymm: attendDate.slice(0, 7) }],
        ]) {
          try {
            ...
            if (withText) console.log(`    한글 있는 줄 예: ${maskNames(JSON.stringify(withText)).slice(0, 320)}`);
            else if (p.rows[0]) console.log(`    첫 줄: ${maskNames(JSON.stringify(p.rows[0])).slice(0, 320)}`);
            if (!p.ok) console.log(`    (응답 앞부분: ${maskNames(r.text).slice(0, 160)})`);
          } catch (e) {
            console.log(`  [${path}] → 실패: ${String(e).slice(0, 140)}`);
          }
        }
      } else {
